...
_FIELDS = ["account_id", "ownership", "region", "persona", "gmv_total", "segment",
           "health", "play", "feature", "channel", "expected_value", "prize_type",
           "gmv_at_stake", "action", "reason", "draft"]
...
def write_html(store: Store, report: RunReport, out_dir: Path) -> Path:
    ...
    for r in queue:
        rows.append(f"""<tr>
          ...
          <td class="num">£{(r['expected_value'] or 0):,.0f}</td>
          <td class="num">£{(r['gmv_at_stake'] or 0):,.0f}<br><span class="muted">{html.escape(r.get('prize_type') or '')}</span></td>
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  <div class="cards">
    ...
    <div class="card"><div class="big">£{report.expected_value_total:,.0f}</div><div class="lbl">expected value (risk-adjusted, 6mo)</div></div>
  </div>
  <div class="chips"><b>Segments:</b> {chips(segs)}</div>
  <div class="chips"><b>Plays:</b> {chips(plays)}</div>
  <table>
    <thead><tr><th>Account</th><th>Segment</th><th>Play</th><th>EV (rank)</th><th>Prize</th><th>Why</th><th>Drafted next best action</th></tr></thead>
